#!/usr/bin/env python3
"""
TRIDENT-based Supervised Attention Inference Script for TCGA-PRAD, GTEx_Prostate, or Combined

Runs inference on WSI feature files using a trained ABMIL supervised attention model.
Supports attention heatmap generation and optional WSI overlay visualization.

Usage:
    # Basic inference on all features
    python inference_supervised_attention_trident.py \
        --checkpoint output/supervised_attention/supervised_attention_XXXX/best_model.pt \
...
DEFAULT_FEATS_DIR = DEFAULT_GTEX_FEATS_DIR
DEFAULT_LABELS_PATH = DEFAULT_GTEX_LABELS_PATH

CLASS_NAMES = {0: 'Benign', 1: 'Cancer'}
...
def _resolve_finetuned_checkpoint(checkpoint_path: Path) -> Optional[Path]:
    """Pick GTEx fine-tuned checkpoint if present."""
    candidate = checkpoint_path.parent / 'best_model_finetuned_gtex.pt'
    return candidate if candidate.exists() else None


def load_features(h5_path: Path, max_patches: Optional[int] = None) -> Tuple[torch.Tensor, Optional[np.ndarray], dict]:
...
def load_labels_file(labels_path: Path) -> pd.DataFrame:
    """Load labels for evaluation (CSV or TSV)."""
    ...
    if 'slide_id' in df.columns and 'label' not in df.columns:
        df = df.assign(label=1)
        return df[['slide_id', 'label']]
    raise ValueError("Labels TSV must contain 'slide_id' column.")
...
def generate_attention_heatmap(
    ...
    if output_path:
        if show_colorbar:
            ...
            cbar.set_ticklabels(['Noncancer', 'Medium', 'Cancer'])
    ...
    for att_path in tqdm(attention_files, desc="Generating heatmaps"):
        ...
        try:
            generate_attention_heatmap(
                ...
                gaussian_sigma, smooth, show_colorbar
            )
        except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(
        description='TRIDENT ABMIL Supervised Attention Inference',
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    
    parser.add_argument('--checkpoint', type=str, required=True,
                        help='Path to model checkpoint (best_model.pt)')
    parser.add_argument('--use_finetuned_gtex', action='store_true',
                        help='Use best_model_finetuned_gtex.pt if available')
    parser.add_argument(
        '--dataset',
        type=str,
        default='combined',
        choices=['combined', 'tcga', 'gtex'],
        help='Which dataset to run inference on: combined, tcga, or gtex'
    )
    parser.add_argument('--feats_dir', type=str, default=DEFAULT_FEATS_DIR,
                        help='Directory containing H5 feature files (overrides --dataset)')
    parser.add_argument('--tcga_feats_dir', type=str, default=DEFAULT_TCGA_FEATS_DIR,
                        help='TCGA features directory')
    parser.add_argument('--gtex_feats_dir', type=str, default=DEFAULT_GTEX_FEATS_DIR,
                        help='GTEx features directory')
    ...
    parser.add_argument('--output_dir', type=str, default='output/supervised_attention_inference',
    ...
    parser.add_argument('--tcga_labels_path', type=str, default=DEFAULT_TCGA_LABELS_PATH,
                        help='TCGA labels TSV path (slide_id only allowed)')
    parser.add_argument('--gtex_labels_path', type=str, default=DEFAULT_GTEX_LABELS_PATH,
                        help='GTEx labels CSV path')
    ...
    if args.use_finetuned_gtex:
        finetuned = _resolve_finetuned_checkpoint(checkpoint_path)
        if finetuned is not None:
            checkpoint_path = finetuned
            print(f"Using GTEx fine-tuned checkpoint: {checkpoint_path}")
    ...
    use_custom_feats_dir = args.feats_dir and args.feats_dir != DEFAULT_FEATS_DIR
    if use_custom_feats_dir:
        feats_dirs = [feats_dir]
        dataset_mode = 'custom'
    else:
        dataset_mode = args.dataset.lower()
        if dataset_mode == 'tcga':
            feats_dirs = [Path(args.tcga_feats_dir)]
        elif dataset_mode == 'gtex':
            feats_dirs = [Path(args.gtex_feats_dir)]
        else:
            feats_dirs = [Path(args.tcga_feats_dir), Path(args.gtex_feats_dir)]
    
    if slide_ids:
        resolved = {}
        for sid in slide_ids:
            for d in feats_dirs:
                candidate = d / f"{sid}.h5"
                if candidate.exists():
                    resolved[sid] = candidate
                    break
        h5_paths = list(resolved.values())
        print(f"\nFound {len(h5_paths)} / {len(slide_ids)} requested slides")
    else:
        resolved = {}
        for d in feats_dirs:
            for h5_path in sorted(d.glob('*.h5')):
                if h5_path.stem not in resolved:
                    resolved[h5_path.stem] = h5_path
        h5_paths = list(resolved.values())
    ...
    if args.labels_path:
        ...
        if labels_path.exists():
            ...
            labels_df = None
        if labels_df is None and dataset_mode in {'tcga', 'gtex', 'combined'}:
            labels_frames = []
            if dataset_mode in {'tcga', 'combined'}:
                tcga_path = Path(args.tcga_labels_path)
                if tcga_path.exists():
                    try:
                        labels_frames.append(load_labels_file(tcga_path))
                    except Exception as exc:
                        print(f"Warning: Could not load TCGA labels: {exc}")
            if dataset_mode in {'gtex', 'combined'}:
                gtex_path = Path(args.gtex_labels_path)
                if gtex_path.exists():
                    try:
                        labels_frames.append(load_labels_file(gtex_path))
                    except Exception as exc:
                        print(f"Warning: Could not load GTEx labels: {exc}")
            if labels_frames:
                labels_df = pd.concat(labels_frames, ignore_index=True).drop_duplicates('slide_id')
