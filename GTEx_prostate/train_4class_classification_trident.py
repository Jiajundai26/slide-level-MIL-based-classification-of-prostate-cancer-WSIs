...
4-Class Classification (Prostate Grading):
    - Class 0: Normal (GTEx benign prostate tissue only)
...
Combines TCGA-PRAD (cancer) and GTEx_Prostate benign-only dataset.
...
def load_gtex_benign_labels(labels_path: Path) -> pd.DataFrame:
    """
...
def train_4class(
    ...
    def _first_existing_h5(slide_to_dir_map: Dict[str, str]) -> Optional[Path]:
        for slide_id, dir_path in slide_to_dir_map.items():
            h5_path = Path(dir_path) / f"{slide_id}.h5"
            if h5_path.exists():
                return h5_path
        for dir_path in set(slide_to_dir_map.values()):
            for h5_file in Path(dir_path).glob("*.h5"):
                return h5_file
        return None
    
    sample_h5 = _first_existing_h5(slide_to_dir)
    ...
    def _can_stratify(labels_list: List[int]) -> bool:
        counts = defaultdict(int)
        for lbl in labels_list:
            counts[lbl] += 1
        return min(counts.values()) >= 2 if counts else False
    
    # First split: train+val vs test
    if _can_stratify(all_lbls):
        ...
        print("Warning: too few samples per class for stratified test split; using random split.")
        train_val_ids, test_ids, train_val_lbls, test_lbls = train_test_split(
            all_ids, all_lbls,
            test_size=test_split,
            random_state=SEED
    ...
    if _can_stratify(train_val_lbls):
        ...
        print("Warning: too few samples per class for stratified val split; using random split.")
        train_ids, val_ids, train_lbls, val_lbls = train_test_split(
            train_val_ids, train_val_lbls,
            test_size=val_ratio,
            random_state=SEED
...
Data Distribution:
    ...
    # Load GTEx data (benign-only slides: label 0)
    ...
    if gtex_labels_path.exists():
        ...
        print(f"\nGTEx Prostate: {len(gtex_df)} benign slides loaded")
        
        # Keep only benign GTEx as Normal (label 0)
