#!/usr/bin/env python3
"""
TRIDENT-based Training with Supervised Attention for Combined or Single Datasets

This script extends the standard ABMIL training with attention supervision
using GeoJSON cancer annotations (TCGA) and negative supervision on normal
slides (GTEx benign). The model learns to focus on annotated cancer regions
while keeping attention diffuse on benign tissue.
...
Usage:
    python train_supervised_attention_trident.py --train --epochs 50 --dataset combined
    
    # Adjust attention supervision weight
    python train_supervised_attention_trident.py --train --attention_lambda 1.0 --dataset tcga
    
    # GTEx only
    python train_supervised_attention_trident.py --train --dataset gtex

    # Increase normal-slide attention regularization + GTEx fine-tune
    python train_supervised_attention_trident.py --train --dataset combined \
        --normal_weight 1.0 --finetune_gtex --finetune_epochs 10
...
def attention_supervision_loss(
    ...
    Encourages:
        ...
        - Diffuse attention on normal slides (all patches normal)
    ...
    Args:
        ...
        normal_weight: Weight for normal-slide attention regularization
    ...
    for i in range(batch_size):
        ...
        if n_cancer > 0 and n_normal > 0:
            ...
            # Normal slide: encourage diffuse (near-uniform) attention
            target = torch.full_like(att, 1.0 / att.numel())
            uniform_loss = F.mse_loss(att, target)
            total_loss += normal_weight * uniform_loss
            n_valid += 1
...
def train_epoch(
    ...
    for batch in pbar:
        ...
        att_loss = attention_supervision_loss(
            attention, instance_labels, mask, has_instance_labels, normal_weight
        )
...
def train_supervised_attention(
    ...
    device: str = 'cuda:0',
    num_workers: int = 4,
    finetune_gtex: bool = False,
    finetune_epochs: int = 10,
    finetune_lr: float = 5e-5,
    finetune_attention_lambda: Optional[float] = None
):
    ...
    def _first_existing_h5(feats_dir: Path, slide_ids: List[str]) -> Optional[Path]:
        for slide_id in slide_ids:
            h5_path = feats_dir / f"{slide_id}.h5"
            if h5_path.exists():
                return h5_path
        return None
    
    sample_h5 = _first_existing_h5(tcga_feats_dir, tcga_slide_ids)
    if sample_h5 is None:
        sample_h5 = _first_existing_h5(gtex_feats_dir, gtex_slide_ids)
    if sample_h5 is None:
    ...
    if sample_h5 is None:
        for h5_file in gtex_feats_dir.glob('*.h5'):
            ...
            break
    if sample_h5 is None:
        raise FileNotFoundError("No .h5 feature files found for selected dataset(s).")
    ...
    # If the temp split is too small for stratification, fall back to non-stratified split
    temp_label_counts = defaultdict(int)
    for lbl in temp_lbls:
        temp_label_counts[lbl] += 1
    min_temp_count = min(temp_label_counts.values()) if temp_label_counts else 0
    if min_temp_count < 2:
        print("Warning: too few samples for stratified val/test split; using random split.")
        val_ids, test_ids, _, _ = train_test_split(
            temp_ids, temp_lbls, test_size=0.5, random_state=SEED
        )
    else:
        val_ids, test_ids, _, _ = train_test_split(
    ...
    if finetune_attention_lambda is None:
        finetune_attention_lambda = attention_lambda
    ...
    for epoch in range(1, epochs + 1):
        ...
        cls_loss, att_loss, train_acc, train_bal_acc = train_epoch(
            model, train_loader, criterion, optimizer, device, attention_lambda, normal_weight
        )
    ...
    # Optional GTEx-only fine-tune after combined training
    if finetune_gtex:
        if len(train_gtex) == 0:
            print("\nWarning: No GTEx slides available for fine-tuning. Skipping.")
            return test_results
        
        print("\n" + "=" * 70)
        print("GTEx Fine-tuning")
        print("=" * 70)
        
        # Reload best model from combined training
        checkpoint = torch.load(output_dir / 'best_model.pt', map_location=device)
        model.load_state_dict(checkpoint['model_state_dict'])
        
        # Build GTEx-only datasets using the same split
        finetune_train = MultiSourceSupervisedDataset(
            tcga_feats_dir, gtex_feats_dir, [], train_gtex,
            labels, annotations_dir, PATCH_SIZE, max_patches, subsample=True
        )
        finetune_val = MultiSourceSupervisedDataset(
            tcga_feats_dir, gtex_feats_dir, [], val_gtex,
            labels, annotations_dir, PATCH_SIZE, max_patches, subsample=False
        )
        finetune_test = MultiSourceSupervisedDataset(
            tcga_feats_dir, gtex_feats_dir, [], test_gtex,
            labels, annotations_dir, PATCH_SIZE, max_patches, subsample=False
        )
        
        finetune_train_loader = DataLoader(
            finetune_train, batch_size=batch_size, shuffle=True,
            num_workers=num_workers, collate_fn=collate_fn,
            pin_memory=(device.type == 'cuda')
        )
        finetune_val_loader = DataLoader(
            finetune_val, batch_size=1, shuffle=False,
            num_workers=num_workers, collate_fn=collate_fn
        )
        finetune_test_loader = DataLoader(
            finetune_test, batch_size=1, shuffle=False,
            num_workers=num_workers, collate_fn=collate_fn
        )
        
        # Recompute class weights on GTEx train split
        finetune_lbls = [labels[s] for s in train_gtex]
        n_pos = sum(finetune_lbls)
        n_neg = len(finetune_lbls) - n_pos
        pos_weight = torch.tensor([n_neg / max(n_pos, 1)]).to(device)
        print(f"GTEx class balance: Normal={n_neg}, Cancer={n_pos}")
        print(f"GTEx positive weight: {pos_weight.item():.2f}")
        
        finetune_criterion = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
        finetune_optimizer = optim.AdamW(model.parameters(), lr=finetune_lr, weight_decay=weight_decay)
        finetune_scheduler = optim.lr_scheduler.CosineAnnealingLR(
            finetune_optimizer, T_max=finetune_epochs, eta_min=finetune_lr * 0.01
        )
        
        best_val_auc = -1.0
        best_epoch = 0
        finetune_history = {'train': [], 'val': []}
        
        for epoch in range(1, finetune_epochs + 1):
            print(f"\nFine-tune Epoch {epoch}/{finetune_epochs}")
            print("-" * 50)
            
            cls_loss, att_loss, train_acc, train_bal_acc = train_epoch(
                model, finetune_train_loader, finetune_criterion, finetune_optimizer,
                device, finetune_attention_lambda, normal_weight
            )
            
            val_results = evaluate(model, finetune_val_loader, finetune_criterion, device)
            ...
            finetune_history['train'].append({
                'epoch': epoch, 'cls_loss': cls_loss, 'att_loss': att_loss,
                'accuracy': train_acc, 'balanced_accuracy': train_bal_acc
            })
            finetune_history['val'].append({
                'epoch': epoch, **{k: v for k, v in val_results.items() if k != 'predictions'}
            })
            ...
            if val_results['auc'] > best_val_auc:
                ...
                torch.save({
                    ...
                    'config': {
                        ...
                        'hidden_dim': hidden_dim,
                        'attention_dim': attention_dim,
                        'attention_lambda': finetune_attention_lambda,
                        'normal_weight': normal_weight,
                        'mode': 'cancer_vs_noncancer',
                        'finetune': 'gtex'
                    }
                }, output_dir / 'best_model_finetuned_gtex.pt')
                print(f"  -> New best GTEx fine-tune model! (AUC: {best_val_auc:.4f})")
            ...
            }, output_dir / 'latest_checkpoint_finetune.pt')
        
        with open(output_dir / 'history_finetune_gtex.json', 'w') as f:
            json.dump(finetune_history, f, indent=2)
        
        if (output_dir / 'best_model_finetuned_gtex.pt').exists():
            print("\n" + "=" * 70)
            print(f"Loading GTEx fine-tuned model (epoch {best_epoch}, AUC: {best_val_auc:.4f})")
            print("=" * 70)
            checkpoint = torch.load(output_dir / 'best_model_finetuned_gtex.pt', map_location=device)
            model.load_state_dict(checkpoint['model_state_dict'])
        
        finetune_test_results = evaluate(model, finetune_test_loader, finetune_criterion, device)
        with open(output_dir / 'test_results_finetune_gtex.json', 'w') as f:
            json.dump(finetune_test_results, f, indent=2)
        
        pred_df = pd.DataFrame({
            'slide_id': finetune_test_results['predictions']['slide_ids'],
            'true_label': finetune_test_results['predictions']['labels'],
            'predicted': finetune_test_results['predictions']['predictions'],
            'probability': finetune_test_results['predictions']['probabilities']
        })
        pred_df.to_csv(output_dir / 'test_predictions_finetune_gtex.csv', index=False)
        
        print(f"\nGTEx fine-tune results saved to: {output_dir}")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default='combined',
        choices=['combined', 'tcga', 'gtex'],
        help='Which dataset to train on: combined, tcga, or gtex'
    )
    ...
    parser.add_argument('--normal_weight', type=float, default=0.1,
                        help='Weight for normal-slide attention regularization')
    parser.add_argument('--max_patches', type=int, default=4096)
    parser.add_argument('--num_workers', type=int, default=4)
    parser.add_argument('--device', type=str, default='cuda:0')
    
    # Fine-tuning
    parser.add_argument('--finetune_gtex', action='store_true',
                        help='Fine-tune on GTEx after combined training')
    parser.add_argument('--finetune_epochs', type=int, default=10,
                        help='Number of GTEx fine-tune epochs')
    parser.add_argument('--finetune_lr', type=float, default=5e-5,
                        help='Learning rate for GTEx fine-tuning')
    parser.add_argument('--finetune_attention_lambda', type=float, default=None,
                        help='Attention lambda for GTEx fine-tuning (defaults to attention_lambda)')
    ...
    if dataset_mode in {'combined', 'tcga'}:
        ...
        if tcga_labels_path.exists():
            ...
            labels.update({row['slide_id']: 1 for _, row in tcga_df.iterrows()})  # All TCGA = cancer
    ...
    if dataset_mode in {'combined', 'gtex'}:
        ...
        if gtex_labels_path.exists():
            ...
            if dataset_mode == 'gtex':
                print(f"Error: GTEx labels not found: {gtex_labels_path}")
                return
            print(f"Warning: GTEx labels not found, training on TCGA only")
            gtex_slide_ids = []
    
    if len(tcga_slide_ids) == 0 and len(gtex_slide_ids) == 0:
        print("Error: No slides loaded for the selected dataset mode.")
        return
    ...
    train_supervised_attention(
        ...
        finetune_gtex=args.finetune_gtex,
        finetune_epochs=args.finetune_epochs,
        finetune_lr=args.finetune_lr,
        finetune_attention_lambda=args.finetune_attention_lambda
    )
