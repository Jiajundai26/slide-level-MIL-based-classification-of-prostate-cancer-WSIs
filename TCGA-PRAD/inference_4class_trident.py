...
def get_4class_colormap():
    """Create a colormap suitable for 4-class attention visualization."""
    # Use viridis which works well for continuous attention values
    ...
    if output_path:
        if show_colorbar or show_prediction:
            ...
            if show_prediction:
                ...
                title += f"P(Normal)={prob_normal:.3f}, P(G3)={prob_g3:.3f}, "
                title += f"P(G4)={prob_g4:.3f}, P(G5)={prob_g5:.3f}"
...
def evaluate_predictions(
    ...
    if 'prob_g3' in merged.columns:
        # Get probabilities for classes present in the data
...
def plot_roc_curves(
    ...
    for i, c in enumerate(classes):
        if c < n_classes and c < y_true_bin.shape[1]:
...
Classes:
    0: Normal (non-cancer) - GTEx prostate tissue
    ...
    parser.add_argument('--no_prediction_label', action='store_true',
                        help='Disable prediction label on heatmaps')
    ...
    if args.dataset in ['gtex', 'combined']:
        ...
        if gtex_feats_dir.exists():
            ...
            # Load GTEx labels for evaluation (all Normal = 0)
            if args.evaluate:
                ...
                if gtex_labels_path.exists():
                    ...
                    for _, row in gtex_df.iterrows():
                        # GTEx is all Normal (class 0)
                        ground_truth_labels[row['slide_id']] = 0
        else:
        ...
        for att_path in tqdm(attention_files, desc="Generating heatmaps"):
            ...
            try:
                generate_attention_heatmap(
                    ...
                    not args.no_colorbar, not args.no_prediction_label
                )
            except Exception as e:
                print(f"Error generating heatmap for {slide_id}: {e}")
